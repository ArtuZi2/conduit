...
"""def test_save():
    time.sleep(2)
    driver.get("http://localhost:1667/#/settings")
    settings = driver.find_element_by_xpath("//a[@href='#/settings']")
    settings.click()
    time.sleep(2)

    data_list = []
    username = driver.find_element_by_xpath("//fieldset[2]/input[' + str(r) + ']")
    emil = driver.find_element_by_xpath("//fieldset[4]/input[' + str(r) + ']")
