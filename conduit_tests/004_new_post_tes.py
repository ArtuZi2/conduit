...
#New Article button
continue_link = browser.find_element_by_xpath("//a[@href='#/editor']").click()

def test_new_post(browser):

    time.sleep(2)
    article = browser.find_element_by_xpath("//fieldset[1]/input")
    article.send_keys("Spring")
    time.sleep(3)
    browser.find_element_by_xpath("//fieldset[2]/input").send_keys("About my favorite season")
    time.sleep(2)
    browser.find_element_by_xpath("//fieldset[3]/textarea").send_keys("Spring is my life.")
    time.sleep(2)
    browser.find_element_by_xpath("//fieldset[4]/div/div/ul/li/input").send_keys("Spring\n", "life\n", "season\n",
                                                                                "favorite\n")
    time.sleep(4)
    browser.find_element_by_xpath("//button[@class='btn btn-lg pull-xs-right btn-primary']").click()

time.sleep(2)

my_feed = browser.find_element_by_xpath("//a[@href='#/my-feed']")
my_feed.click()
